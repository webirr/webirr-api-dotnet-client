...
Retry only transient platform failures with exponential backoff and jitter: connection errors, timeouts, and HTTP 5xx / 429 / 408. Use `TransientErrors.IsTransient(exception)` to apply that rule. Never retry other 4xx responses.
...
{
    if (TransientErrors.IsTransient(ex))
    {
        // Transient platform error: timeout, network/connection failure,
        // HTTP 5xx, 429, or 408.
